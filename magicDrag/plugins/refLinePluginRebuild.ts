...
class RefLine implements Plugin {
	...
	constructor(private readonly options: { gap?: number, adsorbAfterStopDiff?: boolean } = {}) {
		...
		this.isHasAdsorbElement = () => {
			return this.isHasAdsorbElementX || this.isHasAdsorbElementY
		}
	}
	init() {
		this.lines = getLines()
	...
	drag({ elementParameter, stateParameter, globalDataParameter, optionParameter }: Parameter, { movement, _updateContourPointPosition, _updateState }) {
		...
		console.log(this.isHasAdsorbElement, 'this.isHasAdsorbElement')
		// 遍历nodeList
			...
			for (let key in conditions) {
				conditions[key].forEach((condition) => {
					...
					if (!this.isHasAdsorbElementX && key === 'left' || !this.isHasAdsorbElementY && key === 'top') {
						condition.lineNode.show()
					} else if (this.isHasAdsorbElementX && key === 'left') {
						// 如果已经有一条线满足吸附条件了，那么同水平方向必须宽高相等才会显示其他线
						condition.equality && condition.lineNode.show()
					} else if (this.isHasAdsorbElementY && key === 'top') {
						condition.equality && condition.lineNode.show()
					}
					this[key === 'top' ? 'isHasAdsorbElementY' : 'isHasAdsorbElementX'] = true
				})
			}
